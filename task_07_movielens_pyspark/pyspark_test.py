...
#################################################################
#################################################################
# You must supply the path to the ml-100k folder as an argument #
#################################################################
#################################################################
        ...
        if idx in selection:
            if types[idx] == 'int': 
                ...
                try:   
                    ...
                    result.append(datetime.min) #keeps dtype consistency when data is missing and is easy to filter out.               
            elif types[idx] == 'date':
                try:
...
def main():
    ...
    genre_range.insert(0,2)   
    genre_range.insert(0,0)    
        
    # Parse files
    item_g = sq.createDataFrame(item_f.map(lambda f: parseFile(f, item_t, genre_range,'|')),genre_cols)
    ...
    print("\nQuestion 3:")
    data_mid_genre = data_g.select('movie_id').join(item_g, ['movie_id'], how='full')
    data_mid_genre.select(data_mid_genre.columns[1:]).groupBy().sum().show(truncate=False)
    
    # Q4.Print oldest movie with a 5 rating.
    print("\nQuestion 4:")
    data_mid_date = data_g.select('movie_id','rating').filter(data_g.rating == 5)\
    .join(item_g.select('movie_id','release_date'), ['movie_id'], how='full')
    ...
    print("\nQuestion 5:")
    top_ten_genres = top_ten_ratings.join(item_g, ['movie_id'],how='left')
